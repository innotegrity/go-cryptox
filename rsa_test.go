...
func TestParsePublicKeyFromCertificateFailure(t *testing.T) {
	...
	expected := "failed to parse public key: no certificate was provided"
	_, err := cryptox.ParsePublicKeyFromCertificate(ctx, nil)
	if err == nil {
	...
	expected = "failed to parse public key: public key does not appear to be in RSA format"
	_, err = cryptox.ParsePublicKeyFromCertificate(ctx, &x509.Certificate{})
	if err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
...
func TestSignFailure(t *testing.T) {
	...
	expected := "failed to sign contents: no content was provided"
	_, err := cryptox.Sign(ctx, nil, nil)
	if err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
	...
	expected = "failed to sign contents: no private key was provided"
	_, err = cryptox.Sign(ctx, []byte(TestContents), nil)
	if err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
	...
	_, err = cryptox.Sign(ctx, []byte(TestContents), privateKey)
	if err == nil {
...
func TestSignVerifyFailure(t *testing.T) {
	...
	publicKey, err := cryptox.ParsePublicKeyFromCertificate(ctx, cert)
	if err != nil {
	...
	// sign the contents
	signature, err := cryptox.Sign(ctx, []byte(TestContents), privateKey)
	if err != nil {
	...
	if err := cryptox.Verify(ctx, []byte(TestContents), signature, publicKey); err == nil {
...
func TestSignVerifySuccess(t *testing.T) {
	...
	// sign the contents
	signature, err := cryptox.Sign(ctx, []byte(TestContents), key)
	if err != nil {
		t.Fatalf("error: failed to generate signature: %s", err.Error())
	} else {
		t.Log("  generated signature")
...
func TestVerifyFailure(t *testing.T) {
	...
	t.Log("*** testing nil contents ***")
	expected := "failed to verify signature: no content was provided"
	if err := cryptox.Verify(ctx, nil, nil, nil); err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
		if errMsg != expected {
	...
	expected = "failed to verify signature: no signature was provided"
	if err := cryptox.Verify(ctx, []byte(TestContents), nil, nil); err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
		if errMsg != expected {
	...
	expected = "failed to verify signature: no public key was provided"
	if err := cryptox.Verify(ctx, []byte(TestContents), []byte{}, nil); err == nil {
		t.Errorf("error: got nil, expected %s", expected)
	} else {
		errMsg := err.Error()
		if errMsg != expected {
	...
	if err := cryptox.Verify(ctx, []byte(TestContents), []byte{}, publicKey); err == nil {
		t.Errorf("error: got nil, expected error")
	} else {
