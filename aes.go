...
func DecryptString(ctx context.Context, ciphertext, key string) (string, errorx.Error) {
	...
	if err != nil {
		e := NewDecryptionError("failed to decode ciphertext string", err)
	...
	if err != nil {
		e := NewDecryptionError("failed to generate new cipher", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
	...
	if err != nil {
		e := NewDecryptionError("failed to generate GCM", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
	...
	if err != nil {
		e := NewDecryptionError("failed to decrypt the data", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
...
func EncryptString(ctx context.Context, plaintext, key string) (string, errorx.Error) {
	...
	if key == "" {
		...
		if _, err := rand.Read(cipherKey); err != nil {
			e := NewEncryptionError("failed to generate random key", err)
			logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
			return "", e
		}
	}
	...
	block, err := aes.NewCipher(sha[0:32])
	if err != nil {
		e := NewEncryptionError("failed to generate a new cipher", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
	...
	aesGCM, err := cipher.NewGCM(block)
	if err != nil {
		e := NewEncryptionError("failed to generate GCM", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
	...
	if _, err = io.ReadFull(rand.Reader, nonce); err != nil {
		e := NewEncryptionError("failed to generate nonce from GCM", err)
		logger.Error(e.Msg(), slogx.ErrX(errAttr, e))
		return "", e
	}
